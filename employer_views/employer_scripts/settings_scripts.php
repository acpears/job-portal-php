...
function sendManualTransactionEmail($email,$amount,$balance){
    ...
    $receiver = $email;
    $subject = "Confirmation of manual payment made to employer account " . $email;
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(isset($_POST['cname'])){
        ...
        $query_updateDetails = "UPDATE Employer SET name = '$cname', email = '$email' WHERE id = $userId";
        ...
        if(mysqli_query($db, $query_updateDetails)){
            ...
            if(mysqli_errno($db) == 1062){
                ...
                header("location: ../employer_settings.php");
                exit();  
            }
    ...
    if(isset($_POST['plans'])){
        ...
        // Query to update employer plan
        $query_changePlan= "UPDATE Employer SET plan_name = '$plan' WHERE id = $userId";
    ...
    if(isset($_POST['makePayment'])){
        ...
        if($wallet['default_payment_id']){
            if(mysqli_query($db, $query_makePayment)){
                ...
                // enable account if balance is less then 100
                if($balance <= 0){
                    mysqli_query($db,"UPDATE Employer SET enabled = 1 WHERE id = $userId");
                    $_SESSION['enabled'] = 1;
                }
    ...
    if(isset($_POST['delete'])){
        ...
        $query_deleteUser = "DELETE FROM Employer WHERE id = $id";
    ...
    if(isset($_POST['deleteCategory'])){
        $name = mysqli_real_escape_string($db, $_POST['deleteCategory']);
        $query_deleteJobCategory = "DELETE FROM Job_Category WHERE name = '$name'";

        mysqli_query($db,$query_deleteJobCategory);
    }

    if(isset($_POST['addCategory'])){
        $name = mysqli_real_escape_string($db, $_POST['addCategory']);
        $query_addJobCategory = "INSERT INTO Job_Category (name) VALUES ('$name')";
        mysqli_query($db,$query_addJobCategory);

        if(mysqli_errno($db) === 1062){
            $_SESSION['err'] = "Category already exists";
        }
