...
$query_getAllUsers = "SELECT *, e.id AS userId  FROM Employer e JOIN (SELECT id, balance FROM Wallet) w ON e.wallet_id = w.id";
$result_getAllUsers = mysqli_query($db, $query_getAllUsers);

$query_getOutsandingBalances = "SELECT * FROM Employer e
                                            JOIN Wallet w ON e.wallet_id = w.id
                                            WHERE w.balance > 0";
$result_getOutsandingBalances = mysqli_query($db, $query_getOutsandingBalances);
...
<body>
    ...
    <div class="container-fluid">
        <div class="container-fluid">
            <div class="row my-3">
                <div class="col-4">
                    <h1 class="mb-0">Registered Employers</h1>
                </div>
        ...
        <table class="table table-bordered table-hover">
            <thead class="thead-light">
                <tr>
                    ...
                    <th style="width: 100px;">Postings</th>
            ...
            <tbody>
                <?php while ($row = mysqli_fetch_assoc($result_getAllUsers)) {
                    if($row['balance'] >= 0.01){
                        array_push($outstandingBalances, $row);
                    }
                    $userId = $row['id'];
                    $query_numberOfPostings = "SELECT COUNT(*) AS value FROM Job_Posting WHERE employer_id = $userId";
                    $numberOfPostings = mysqli_fetch_assoc(mysqli_query($db, $query_numberOfPostings));
                    

                ?>
                    <tr>
                        <th class="text-center"><?= $row['userId'] ?></th>
                        <th><?= $row['name'] ?></th>
                        <th><?= $row['email'] ?></th>
                        <th><?= $row['plan_name'] ?></th>
                        <th class="text-center"><?= round($row['balance'], 2) ?>$</th>
                        <th class="text-center"><?= $numberOfPostings['value'] ?></th>
                        <th>
                            <form action="./admin_scripts/employer_scripts.php" method="post">
                    ...
                    <tr>
                        ...
                        <th>
                            <form action="./admin_scripts/employer_scripts.php" method="post">
                                ...
                                <input name='cname' value="<?= $row['name']  ?>" hidden>
